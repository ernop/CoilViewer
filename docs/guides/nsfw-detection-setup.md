...
CoilViewer now includes optional NSFW (Not Safe For Work) content detection using ONNX Runtime. This feature uses your GPU for fast inference when a CUDA-capable GPU is available.
...
- With a CUDA-capable GPU, expect sub-100ms inference times per image
