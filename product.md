...
- When navigation reaches the start or end of the current image list, the viewer refreshes the folder once so newly added sorted neighbors can be reached by the next arrow-key movement without continuous polling.
- Zoom and pan input rules are covered by broad self-tests: tiny fit scales are preserved for extremely large images, relative zoom detection works at very small scales, mouse-anchor zooming is checked across many image shapes and viewport positions, and mouse wheel pans while zoomed instead of navigating images.
